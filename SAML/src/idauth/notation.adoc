...
When referring to attributes from the W3C XML Schema Definition Language
<<XMLSchema>>, the following syntax is used:

* `xs:Attribute` - for attributes from the schema namespace
* `xsi:Atribute` - for attributes from the schema instance namespace
...
Relying Party (RP):: A federation member who relies on assurances of identity
